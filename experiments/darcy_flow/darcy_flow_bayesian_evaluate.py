...
if __name__ == '__main__':
	...
	if args.save_all:
		...
		# 	vmin, vmax = torch.min(p_test[idx]), torch.max(p_test[idx])
		# 	plt.subplot(1, 4, 2)
		...
		# 	plt.subplot(1, 4, 4)
		# 	fe.plot(p_pred_bayes_var[idx], V, cmap='magma', colorbar=True)
		...
		# 	fe.plot(K_test[idx], V, cmap='jet', colorbar=True)
		# 	plt.title("Permeability field")
		# 	vmin, vmax = torch.min(u_y_test[idx]), torch.max(u_y_test[idx])
		# 	plt.subplot(1, 4, 2)
		# 	fe.plot(u_y_test[idx], V, cmap='jet', vmin=vmin, vmax=vmax, colorbar=True)
		# 	plt.title("True velocity (y)")
		# 	plt.subplot(1, 4, 3)
		# 	fe.plot(u_y_pred_bayes_mean[idx], V, cmap='jet', vmin=vmin, vmax=vmax, colorbar=True)
		# 	plt.title("Predicted velocity (y)")
		# 	plt.subplot(1, 4, 4)
		# 	fe.plot(u_y_pred_bayes_var[idx], V, cmap='magma', colorbar=True)
		# 	plt.title("Velocity variance (y)")
		# 	plt.tight_layout()
		...
		if os.path.exists("history"):
			...
			plt.yscale('symlog')
			min_val, max_val = min(log_posterior_history), max(log_posterior_history)
